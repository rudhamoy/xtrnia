...
export default function AdminRegistrationsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-black via-gray-900 to-black">
      ...
      <div className="overflow-x-auto rounded-xl shadow border border-gray-700 bg-black/60">
        <table className="min-w-full text-sm text-white">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={10} className="text-center py-8 text-yellow-400">Loading...</td>
              </tr>
            ) : registrations.length === 0 ? (
              <tr>
                <td colSpan={10} className="text-center py-8 text-yellow-400">No registrations found.</td>
              </tr>
            ) : (
              registrations.map((r: any) => (
                <tr key={r.id} className="border-t border-gray-700 hover:bg-yellow-400/10">
      ...
      <div className="mt-8">
        <Link href="/admin/dashboard" className="text-yellow-400 hover:underline">&larr; Back to Dashboard</Link>
      </div>
      </div>
    </div>
  );
